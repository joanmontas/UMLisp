...
eval ( ( A (IdentAtom    v )    ):sp ) s results = let r' = findSexp s v
...
evalCallExpression ( CallExpression ( A ( IdentAtom ">" ) ) arg ) a s results = let ( oldArg, s', evaledGroup ) = evalGroupExpression arg s []
...
evalCallExpression ( CallExpression ( A ( IdentAtom "not" ) ) arg ) a s results = if (length arg == 0) || (length arg > 1)
                                                                                  then ([], s, [A(EvalErrorAtom "eval's not Error: Wrong parameter")])
                                                                                  else let (a', s', r') = eval arg s []
                                                                                       in case (head r') of
                                                                                             (A(EvalErrorAtom v)) -> ([], s, r')
                                                                                             otherwise -> if (isTruthy r')
                                                                                                          then (a, s', [A(NilAtom "NIL")])
                                                                                                          else (a, s', [A(TrueAtom "T")])
evalCallExpression ( CallExpression ( A ( IdentAtom "and"     ) ) arg ) a s results = let (a', s', r') = evalLogicalAnd arg s [] -- TODO(JoanMontas) finish this logical and
                                                                                      in (a, s', r')
evalCallExpression ( CallExpression ( A ( IdentAtom "or"      ) ) arg ) a s results = let (a', s', r') =  evalLogicalOr arg s [] -- TODO(JoanMontas) finish this logical or
                                                                                      in (a, s', r')
evalCallExpression ( CallExpression ( A ( IdentAtom "defvar" ) ) arg ) a s results = if not (length arg >= 1 && length arg < 3) || not(expectSexpression (head arg) identAtom)
                                                                                     then ([], s, [A(EvalErrorAtom "eval's defvar Error: Wrong parameter")])
                                                                                       -- NOTE(JoanMontas) I do not have to evaluate, due to potential lazy evaluation features (more research is needed)
                                                                                     else let ( oldArg, s', evaledGroup ) = evalGroupExpression (tail arg) s []
                                                                                          in if (length evaledGroup > 0) && expectSexpression ( head evaledGroup ) evalErrorAtom
                                                                                             then ( [], s, evaledGroup )
                                                                                             else case (head arg) of
                                                                                                   (A(IdentAtom v)) -> if length evaledGroup > 0
                                                                                                                       then let newScope = insertSexpToGlobalScope s' v (head evaledGroup)
                                                                                                                            in (a, newScope, [A(StringAtom v)])
                                                                                                                       else let newScope = insertSexpToGlobalScope s' v (A(NilAtom "NIL"))
                                                                                                                            in (a, newScope, [A (StringAtom v)])

evalCallExpression ( CallExpression ( A ( IdentAtom "setq" ) ) arg ) a s results = if (length arg < 2) || (length arg > 2) || (not (expectSexpression (head arg) identAtom))
                                                                                   then ([], s, [A(EvalErrorAtom "eval's setq Error: Wrong parameter")])
                                                                                   else case (head arg) of 
                                                                                          A(IdentAtom v) -> let originalValue = findSexp s v
                                                                                                            in if expectSexpression originalValue evalErrorAtom
                                                                                                               then ([], s, [originalValue])
                                                                                                               else let (a', s', r') = eval (tail arg) s []
                                                                                                                    in if expectSexpression (head r') evalErrorAtom
                                                                                                                       then ([], s, r')
                                                                                                                       else let s'' = insertSexpToScope s' v (head r')
                                                                                                                            in (a, s'', r')
                                                                                          otherwise      -> ([], s, [A(EvalErrorAtom "eval's setq Error: Expected an Ident as argument")])
evalCallExpression ( CallExpression ( A ( IdentAtom "progn" ) ) arg ) a s results = if length arg == 0
                                                                                    then (a, s, [A(NilAtom "NIL")])
                                                                                    else evalPrognExpression arg a s []

evalCallExpression ( CallExpression ( A ( IdentAtom "list"    ) ) arg ) a s results = let ( oldArg, s', evaledGroup ) = evalGroupExpression arg s []
                                                                                       in if (length evaledGroup > 0) && expectSexpression ( head evaledGroup ) evalErrorAtom
                                                                                          then ( [], s, evaledGroup )
                                                                                          -- else ( a, s', [ (ListExpression evaledGroup)])         -- NOTE(JoanMontas) just lazily evaluate them... lets mess around
                                                                                          else if length evaledGroup == 0
                                                                                               then (a, s, [ListExpression []])
                                                                                               -- else (a, s, [ (ListExpression arg)])       -- NOTE(JoanMontas) hmmmmm lets remove lazy eval until I research more
                                                                                               else (a, s, [ (ListExpression evaledGroup)])

evalCallExpression ( CallExpression ( A ( IdentAtom "nth"     ) ) arg ) a s results = if (length arg /= 2)
                                                                                      then ([], s, [A(EvalErrorAtom "eval's nth Error: Wrong parameter")])
                                                                                      else let (a', s', r') = eval [head arg] s []
                                                                                           in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                                                                                              ...
                                                                                              else if not (expectSexpression (head r' )integerAtom)
                                                                                                   then ( [], s, [A(EvalErrorAtom "eval's nth Error: nth's first argument expects an integer")])
                                                                                                   else let (A(IntegerAtom v)) = head r'
                                                                                                        ...
                                                                                                        in case (head r'') of
                                                                                                         ...
                                                                                                         (ListExpression v') -> if (fromInteger v) >= length v' || (fromInteger v < 0)
                                                                                                                                then ( [], s, [A(EvalErrorAtom "eval's nth Error: ListExpression out of range error")])
                                                                                                                                else let (a''', s''', r''') = eval [v' !! (fromInteger v) ] s'' []
                                                                                                                                    -- TODO(JoanMontas) after we eval the nth S-expression and place its evaluated
...
evalCallExpression ( CallExpression ( A ( IdentAtom "append"  ) ) arg ) a s results = if (length arg /= 2)
                                                                                      then ([], s, [A(EvalErrorAtom "eval's nth Error: Wrong parameter")])
                                                                                      else let (a', s', r') = eval [head arg] s []
                                                                                           in case (head r') of
                                                                                                 A(EvalErrorAtom v) -> ([], s, r' ++ [A(EvalErrorAtom "eval's append Error: Error while evaluating iterable")])
                                                                                                 (ListExpression v) -> let (a'', s'', r'') = eval [arg !! 1] s' []
                                                                                                                       -- TODO(JoanMontas) see how much easier it would have been if I used "cons".
...
evalCallExpression ( CallExpression ( A ( IdentAtom "insertAtNth"     ) ) arg ) a s results = if (length arg /= 3)
...
evalCallExpression ( CallExpression ( A ( IdentAtom "removeNth"  ) ) arg ) a s r = if (length arg /= 1) && (length arg /= 2)
                                                                                   then ([], s, [A(EvalErrorAtom "eval's nth Error: Wrong parameter")])
                                                                                   else let (a', s', r') = eval [head arg] s []
                                                                                        in case (head r') of
                                                                                             A(EvalErrorAtom v) -> ([], s, r' ++ [A(EvalErrorAtom "eval's removeNth Error: Error while evaluating iterable")])
                                                                                             (ListExpression v) -> if length arg  == 1
...
evalCallExpression ( CallExpression ( A ( IdentAtom "replaceAtNth" ) ) arg ) a s results = if (length arg /= 3)
...
evalGroupExpression (a:as) s r = let (a', s', r') = eval [a] s []
                      in if (length r' == 0) || (expectSexpression (head r') evalErrorAtom)
...
evalSum (A(IntegerAtom v)) (as:ass) s r a = let (a', s', r') = eval [as] s []
                           in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
...
evalSub  [a] s r =  let (a', s', r') = eval [a] s []
                           in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                              then ([], s, r' ++ [A(EvalErrorAtom "eval's - Error: Error evaluating argument")])
                              else case (head r') of
...
evalSub  (a:as) s r = let (a', s', r') = eval [a] s []
                             in case (head r') of
                                 A(IntegerAtom v) -> let (a'', s'', r'') = evalSubHelper as s' []
                                                     in case (head r'') of
...
evalSubHelper  [a] s r = let (a', s', r') = eval [a] s []
                                in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                                   then ([], s, r' ++ [A(EvalErrorAtom "eval's - Error: Error evaluating argument")])
                                   else case (head r') of
                                          A(IntegerAtom v') -> ([], s', [A(IntegerAtom ((-1) * v'))])
                                          otherwise -> ([], s, r' ++ [A(EvalErrorAtom "eval's evalSub Error: Given non-numerical value to evalSum '+'" )])
evalSubHelper (a:as) s r = let (a', s', r') = eval [a] s []
                                  in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                                     then ([], s, r' ++ [A(EvalErrorAtom "eval's - Error: Error evaluating argument")])
                                     else case (head r') of
                                             A(IntegerAtom v) -> let (a'', s'', r'') = evalSubHelper as s' []
...
evalMult (A(IntegerAtom v)) (as:ass) s r a = let (a', s', r') = eval [as] s []
                           in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                              then ([], s, r' ++ [A(EvalErrorAtom "eval's * Error: Error evaluating argument")])
                              else case (head r') of
...
evalLesserHelper (A (IntegerAtom v)) ((A (IntegerAtom v')):ys) = if v < v'
...
evalLesserEqualHelper (A (IntegerAtom v)) ((A (IntegerAtom v')):ys) = if v <= v'
...
evalGreaterHelper (A (IntegerAtom v)) ((A (IntegerAtom v')):ys) = if v > v'
...
evalGreaterEqualHelper (A (IntegerAtom v)) ((A (IntegerAtom v')):ys) = if v >= v'
...
evalIfExpression (IfExpression c b e) a s r = let ( a', s', r' ) = eval [c] s []
                                              in if ( length r' > 0) && expectSexpression (head r') evalErrorAtom
...
evalPrognExpression [a] as s r = let (a', s', r') = eval [a] s []
                                 in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
                                    then ([], s, r' ++ [A(EvalErrorAtom "eval's progn Error: Error evaluating bodies 1")])
                                    else (as, s', r')
evalPrognExpression (a:ass) as s r = let (a', s', r') = eval [a] s []
                                     in if (length r' == 0) || expectSexpression (head r') evalErrorAtom
...
evalDefunExpression (FunctionExpression (A(IdentAtom n)) arg bdy) a s r = let newScope = insertSexpToScope s n (FunctionExpression (A(IdentAtom n)) arg bdy)
...
evalCustomCallExpression nme arg a s r = let func = findSexp s nme
                                         in case func of
...
applyArgsToScope ((A(IdentAtom nme)):args) (val:vals) s = let s' = insertSexpToScope s nme val
...
evalLogicalOr (a:as) s r = let ( a', s', r' ) = eval [a] s []
                            in if (length r') == 0 || expectSexpression (head r') evalErrorAtom
