...
data Atom = IntegerAtom    { integerAtomValue    :: Integer }             |
...
parseCallSexpression _ [] _ =          ( [], [ A ( ParseErrorAtom 
...
parseCallSexpression v (t              :ts) s = let      ( t', s', g) = parseGroupSexpression ( t:ts ) s []
...
parseGroupSexpression                   [] s g =          ( [], ( ( A (ParseErrorAtom
                                                                    "parse's parseGroupSexpression Error: S-Expression not closed" ) ):[] ), g )
parseGroupSexpression ((RParenToken c):ts) s g =          ( ts, s, g)
parseGroupSexpression                    t s g = let      ( t', ( s':ss' ) ) = parse t s
...
parseIfStatement t  s = let (t', s') = parse t []
...
parseDefunStatement (t:ts) s = case t of
                                  (IdentToken functionname) -> if (not (lexExpectPeekToken ts lParenToken))
                                                               then ([], [ A (ParseErrorAtom "parse's parseDefunStatement Error: Missing Argument") ])
                                                               else let (t', s', arg) = parseDefunArguments ( tail ts) s []
                                                                    in if (length s' > 0 ) && expectSexpression (head s') parseErrorAtom
                                                                       then ([], s')
                                                                            ...
                                                                            in if (length s'' > 0) && expectSexpression (head s'') parseErrorAtom
                                                                               ...
                                                                               else if not (lexExpectPeekToken t'' rParenToken )
...
parseDefunArguments                   [] s g =          ( [], ( ( A (ParseErrorAtom
                                                                    "parse's parseDefunArguments Error: Arguments not closed" ) ):[] ), g )
...
parseDefunArguments                     t s g = ( [], [ ( A (ParseErrorAtom
                                                                    "parse's parseDefunArguments Error: Unexpected Value" ) ) ], [ ( A (ParseErrorAtom
...
data Scope = LocalScope {local :: (Map.Map String Sexpression), prev  :: Scope } |
             EmptyScope
