...
lexInteger s l =
  if (head s == '-')
    then
      let negativeRslt = lexIntegerHelper (tail s) "-"
       in tokenize
            (fst negativeRslt)
            (l ++ [IntegerToken (read (snd negativeRslt) :: Integer)])
    else
      let positiveRslt = lexIntegerHelper s ""
       in tokenize
            (fst positiveRslt)
...
lexString s l =
  let rslt = lexStringHelper (tail s) ""
   in if (head (fst rslt) /= '"')
